-- Drop and recreate check_address_availability to include postal_code
...
CREATE OR REPLACE FUNCTION public.check_address_availability(p_street text, p_house_number text, p_city text DEFAULT 'Falkensee'::text)
 RETURNS TABLE(street text, house_number text, city text, postal_code text, ausbau_art ausbau_art, ausbau_status ausbau_status, kabel_tv_available boolean, residential_units integer, building_id uuid)
 LANGUAGE plpgsql
 SECURITY DEFINER
